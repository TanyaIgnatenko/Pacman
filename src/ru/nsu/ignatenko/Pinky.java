...
public class Pinky implements Ghost,  ActionListener
{
    ...
    private int directionX = NO_MOTION;
    private int directionY = UP;
    private int nextDirectionX = NO_MOTION;
    private int nextDirectionY = UP;
    ...
    public Pinky(ScreenData screenData_[], int nrow_, int ncollumn_, int blocksize_, int x, int y)
    {
        nrow = nrow_;
        ncollumn = ncollumn_;
    ...
    public void move()
    {
        if(coordX % blocksize == blocksize/2 && coordY % blocksize == blocksize/2)
        {
            ...
            else
            {
                ...
                {
                    posTargetX = pacman.getPosX() + 4*pacman.getDirectionX();
                    posTargetY = pacman.getPosY() + 4*pacman.getDirectionY();
                }
            }
